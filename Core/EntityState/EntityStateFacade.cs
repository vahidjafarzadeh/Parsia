    ...
    public class EntityStateFacade : IBaseFacade<EntityStateDto>
    {
        ...
        public ServiceResult<object> Delete(BusinessParam bp, EntityStateDto dto)
        {
            ...
            try
            {
                ...
                using (var unitOfWork = new UnitOfWork())
                {
                    entityState = unitOfWork.EntityState.Get(p=>p.TableEntityId == dto.TableEntityId && p.TableName == dto.TableName).FirstOrDefault();
                }
        ...
        public ServiceResult<object> GetState(BusinessParam bp, EntityStateDto dto)
        {
            ...
            try
            {
                ...
                using (var unitOfWork = new UnitOfWork())
                {
                    var entityStateList = unitOfWork.EntityState.CreateNativeQuery(queryString, x => new Dictionary<string, object>()
                    {
        ...
        public ServiceResult<object> GetDtoFromRequest(HttpRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(request.Form["tblName"])) dto.TableName = request.Form["tblName"]; else return new ServiceResult<object>(Enumerator.ErrorCode.ApplicationError, "لطفا نام جدول را وارد نمایید");
            if (!string.IsNullOrEmpty(request.Form["tblEntityId"])) dto.TableEntityId = Convert.ToInt64(request.Form["tblEntityId"]); else return new ServiceResult<object>(Enumerator.ErrorCode.ApplicationError, "لطفا شناسه رکورد داخل جدول را وارد نمایید");
