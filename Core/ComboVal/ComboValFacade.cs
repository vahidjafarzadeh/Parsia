            ...
            try
            {
                ...
                using (var unitOfWork = new UnitOfWork())
                {
                    var comboVal = unitOfWork.ComboVal.CreateNativeQuery(queryString, x => new ComboValDto
                    {
                        ...
                        Created = string.IsNullOrEmpty(x[5].ToString())
                            ? (long?) null
                            : Convert.ToDateTime(x[5]?.ToString()).GetTimeStamp(),
                        Updated = string.IsNullOrEmpty(x[6].ToString())
                            ? (long?) null
                            : Convert.ToDateTime(x[6]?.ToString()).GetTimeStamp(),
                        Code = x[7]?.ToString(),
