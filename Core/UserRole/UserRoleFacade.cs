...
namespace Parsia.Core.UserRole
{
    ...
    public class UserRoleFacade : IBaseFacade<UserRoleDto>
    {
        ...
        private static readonly ClassDetails[] ClassDetails =
            (ClassDetails[]) typeof(UserRoleFacade).GetCustomAttributes(typeof(ClassDetails), true);
        ...
        public ServiceResult<object> Save(BusinessParam bp, UserRoleDto dto)
        {
            ...
            try
            {
                ...
                Elastic<UserRoleDto, DataLayer.Model.Core.UserRole.UserRole>.SaveToElastic(userRole,
        ...
        public ServiceResult<object> AutocompleteView(BusinessParam bp)
        {
            ...
            try
            {
                ...
                using (var unitOfWork = new UnitOfWork())
                {
                    var userRoleList = unitOfWork.UserRole.CreateNativeQuery(queryString, x =>
        ...
        public ServiceResult<object> GetDtoFromRequest(HttpRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(request.Form["user"]))
                dto.User = new UserDto {EntityId = Convert.ToInt64(request.Form["user"])};
            else
