    ...
    public class FileFacade : IBaseFacade<FileDto>
    {
        ...
        public ServiceResult<object> GridView(BusinessParam bp)
        {
            try
            {
                var queryString = "select * from (SELECT * FROM [CO].[File]) e" +
                       QueryUtil.GetWhereClause(bp.Clause, QueryUtil.GetConstraintForNativeQuery(bp, "File", false, false, true)) +
                    QueryUtil.GetOrderByClause(bp.Clause);
                var files = _context.File.FromSqlRaw(queryString).OrderBy(x => x.Extension).ThenByDescending(x => x.Created).ToList();
                var lstData = files.Select(file => Copier.GetDto(file)).ToList();
                return lstData.Count <= 0 ? new ServiceResult<object>(new List<FileDto>(), 0) : new ServiceResult<object>(lstData, lstData.Count);
            }
            catch (Exception e)
            {
        ...
        public ServiceResult<object> GetDetails(BusinessParam bp)
        {
            try
            {
                ...
                var pathOfFile = GetPathOfFile(new JsonFileDto() { EntityId = fileDto.EntityId, Path = fileDto.Path, Extension = fileDto.Extension }, bp);
        ...
        public ServiceResult<object> CreateFolder(BusinessParam bp, FolderDto dto)
        {
            try
            {
                if (string.IsNullOrEmpty(dto.FolderName))
                    return ExceptionUtil.ExceptionHandler("لطفا نام پوشه را وارد نمایید", "FileFacade.CreateFolder", bp.UserInfo);
                long? parentId = null;
                ...
                var exists = System.IO.Directory.Exists(webRootPath + fileName);
                if (exists)
                    return ExceptionUtil.ExceptionHandler("نام پوشه تکراری می باشد", "FileFacade.CreateFolder",
                        bp.UserInfo);
                System.IO.Directory.CreateDirectory(webRootPath + fileName);
                ...
                if (!done)
                {
                    return ExceptionUtil.ExceptionHandler("خطا در ذخیره فایل درون دیتابیس", "FileFacade.CreateFolder", bp.UserInfo);
                }
        ...
        public async Task<ServiceResult<object>> CreateFile(BusinessParam bp, HttpRequest request)
        {
            try
            {
                if (string.IsNullOrEmpty(request.Form["name"]))
                    return ExceptionUtil.ExceptionHandler("لطفا نام فایل را وارد نمایید", "FileFacade.CreateFile", bp.UserInfo);
                if (request.Form.Files["file"] == null)
                    return ExceptionUtil.ExceptionHandler("لطفا فایل را انتخاب نمایید", "FileFacade.CreateFile", bp.UserInfo);
                ...
                var fileDto = new FileDto()
                {
                    ...
                    FullTitle = request.Form["name"] + " | " + request.Form["description"] + " | " + request.Form["alt"],
                ...
                if (!done)
                {
                    return ExceptionUtil.ExceptionHandler("خطا در ذخیره فایل درون دیتابیس", "FileFacade.CreateFile", bp.UserInfo);
                }
                Elastic<FileDto, DataLayer.Model.Core.File.File>.SaveToHistory(fileDto, file, file.EntityId, bp);
                Elastic<FileDto, DataLayer.Model.Core.File.File>.SaveToElastic(fileDto, "File", file.EntityId, bp);
                fileDto.EntityId = file.EntityId;
        ...
        private FileDto GetFileFromDataBaseWithId(long entityId, BusinessParam bp)
        {
            ...
            catch (Exception ex)
            {
                if (bp != null)
                {
                    ExceptionUtil.ExceptionHandler(ex, "FileFacade.GetFileFromDataBaseWithId", bp.UserInfo);
                }
                return null;
            }
        }
        ...
        private string GetPathOfFile(JsonFileDto dto, BusinessParam bp)
        {
            try
            {
                return $"/service/file/download/{FileRepositoryFacade.ReplaceInValidCharacter(Encryption.Encrypt(GetJsonData(dto)))}.{dto.Extension}";
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Download(string file, BusinessParam bp, bool tumbnail)
        {
            try
            {
                ...
                return new FileStreamResult(stream, FileRepositoryFacade.MimType[fileDto.Extension]) { FileDownloadName = fileDto.Name + "." + fileDto.Extension };
            }
            catch (Exception e)
            {
