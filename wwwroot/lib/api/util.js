...
var Util = (function () {
     ...
     */
    var diffTime = function (start, end, period) {
        return start - end <= period;
    };

    var loadScript = function (src, currentDomain, dataIndex, callback) {
    ...
    var urlParameter = function (name) {
        var results = new RegExp('[\?&]' + name + '=([^&#]*)').exec(window.location.toString());
        if (results === null) {
            return null;
        } else {
            return results[1] || 0;
    ...
    var validateForm = function (formObject, selectorListForIgnore) { // TODO CheckBox & Radio & Select Validation
        ...
        formObject.find("." + ENVIRONMENT.CssClass.MANDATORY).not('.read-only').each(function () {
            ...
            if (ctx.hasClass(ENVIRONMENT.CssClass.HAS_ERROR)) {
                ...
                var maxLength = formControl.attr('ksun-max-length') ? formControl.attr('ksun-max-length') : Config.INPUT_DEFAULT_MAX_LENGTH;
                var minLength = formControl.attr('ksun-min-length') ? formControl.attr('ksun-min-length') : Config.INPUT_DEFAULT_MIN_LENGTH;
                var type = formControl.attr('ksun-input-type') ? formControl.attr('ksun-input-type') : ENVIRONMENT.InputType.TEXT;
                switch (formControl.get(0).tagName) {
                    case 'INPUT':
                        if (formControl.get(0).type === 'text') {
                            if (formControl.val().length < minLength) {
                                ctx.find('.' + ENVIRONMENT.CssClass.FEED_BACK).text(GeneralBundle.$feedbackMinimumLengthPartOne +
                                    +minLength + GeneralBundle.$feedbackMinimumLengthPartTwo);
                                ctx.addClass('has-error');
                                returnValue = false;
                                return false;
                            }
                            if (formControl.val().length > maxLength) {
                                ctx.find('.' + ENVIRONMENT.CssClass.FEED_BACK).text(GeneralBundle.$feedbackMaximumLengthPartOne +
                                    +maxLength + GeneralBundle.$feedbackMaximumLengthPartTwo);
                                ctx.addClass('has-error');
                                returnValue = false;
                                return false;
                            }
                            if (type === ENVIRONMENT.InputType.NUMBER) {
                                if (ctx.hasClass('double')) {
                                    ...
                                    if (!(regex.test(formControl.val()))) {
                                        ctx.find('.' + ENVIRONMENT.CssClass.FEED_BACK).text(GeneralBundle.$feedbackJustNumber);
                                        ctx.addClass('has-error');
                                        returnValue = false;
                                        return false;
                                    }
                    ...
                    case 'TEXTAREA':
                        ...
                        break;
                    case 'SELECT':
                        if (formControl.find('option:selected').length === 0 || !formControl.val() || formControl.val() < 0) {
    ...
    var setInputValidationListener = function (inputObject) {
        if (inputObject.hasClass('read-only') || inputObject.attr('readonly')) {
            return;
        }
        var maxLength = inputObject.attr('ksun-max-length') ? Number(inputObject.attr('ksun-max-length')) : Config.INPUT_DEFAULT_MAX_LENGTH;
        var minLength = inputObject.attr('ksun-min-length') ? Number(inputObject.attr('ksun-min-length')) : Config.INPUT_DEFAULT_MIN_LENGTH;
        var inputType = inputObject.attr('ksun-input-type') ? inputObject.attr('ksun-input-type') : ENVIRONMENT.InputType.TEXT;
        var itemParent = inputObject.closest('.' + ENVIRONMENT.CssClass.FORM_GROUP);
        inputObject.on('keydown', function (e) {
            var inputLength = 0;
            var keyCode = e.keyCode | e.which;
            // Allow: backspace, delete, tab, escape
            if ($.inArray(keyCode, [ENVIRONMENT.KeyCode.DELETE, ENVIRONMENT.KeyCode.BACK_SPACE, ENVIRONMENT.KeyCode.TAB, ENVIRONMENT.KeyCode.ESC, ENVIRONMENT.KeyCode.ENTER, ENVIRONMENT.KeyCode.CTRL]) !== -1 ||
                // Allow: Ctrl/cmd+A
                    ...
                    if (inputObject.hasClass('double')) {
                        if ((keyCode >= ENVIRONMENT.KeyCode.NUM_0 && keyCode <= ENVIRONMENT.KeyCode.NUM_9) ||
                            ...
                            if (inputObject.val()) {
                                ...
                                var validFractionalLength = inputObject.attr('ksun-fractional-length') ? Number(inputObject.attr('ksun-fractional-length')) : Config.INPUT_DEFAULT_FRACTIONAL_LENGTH;
        ...
        inputObject.on('focusout', function (e) {
            inputObject.data('isSelected', false);
            if (e.target.value.length < minLength) {
                if (itemParent.length && itemParent.hasClass(ENVIRONMENT.CssClass.MANDATORY)) {
                    ...
                    itemParent.find('.' + ENVIRONMENT.CssClass.FEED_BACK).text(GeneralBundle.$feedbackMinimumLengthPartOne +
                        +minLength + GeneralBundle.$feedbackMinimumLengthPartTwo);
                }
            } else {
                if (itemParent.length && itemParent.hasClass(ENVIRONMENT.CssClass.MANDATORY)) {
        ...
        inputObject.on('focusin', function (e) {
            inputObject.data('isSelected', false);
        ...
        inputObject.on('select', function () {
            inputObject.data('isSelected', true);
        });
    };

    var setSelect2ValidationListener = function (selectObject) {
        ...
        selectObject.on('select2:open', function () {
            if (itemParent.length && itemParent.hasClass(ENVIRONMENT.CssClass.MANDATORY)) {
                itemParent.removeClass(ENVIRONMENT.CssClass.HAS_ERROR);
            }
        });
        selectObject.on('select2:close', function () {
    ...
    var isValidPassword = function (password) {
        var regex = new RegExp("((?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[~!@#$%^&*-_?]).{8,20})");
        return regex.test(password);
    };
    var isValidUsername = function (username) {
        var regex = new RegExp("^[a-zA-Z0-9_-]{5,20}$");
        return regex.test(username);
    };
    var isValidName = function (name) {
        var regex = new RegExp("^[\u0620\u0621\u0622\u0624\u0626\u0627\u0628\u067E\u062A-\u062C\u0686\u062D-\u0632\u0698\u0633-\u063A\u0641\u0642\u06A9\u06AF\u0644-\u0648\u06CC\\s]+$");
    ...
    var isValidPhone = function (phone) {
        var regex = new RegExp("(0)?[0-9]{10}");
        return regex.test(phone);
    };
    var isValidEmail = function (email) {
        var regex = new RegExp("^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$"); //[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$
        return regex.test(email);
    };
    var isValidNationalCode = function (nationalCode) {
    ...
    var isValidDomain = function (domain) {
        var regex = new RegExp("^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:([1-9]|[1-5]?[0-9]{2,4}|6[1-4][0-9]{3}|65[1-4][0-9]{2}|655[1-2][0-9]|6553[1-5]))?(\/.*)?$");
        return regex.test(domain);
    };
    var isNumberCardBank = function (number) {
        var regex = new RegExp("^((?:4\d{3})|(?:5[1-5]\d{2})|(?:6011)|(?:3[68]\d{2})|(?:30[012345]\d))[ -]?(\d{4})[ -]?(\d{4})[ -]?(\d{4}|3[4,7]\d{13})$");
        return regex.test(number);
    };

    var isValidIpOfServer = function (ip) {
    ...
    var getTitleOfObjectFromFormat = function (obj, format) {
        var regex = /{([^}]+)}/gm, match;
        var temp = format;
    ...
    var convertFileToBase64 = function (file, onLoad, onError) {
    ...
    const getOperatingSystemFromUserAgent = function (userAgent = navigator.userAgent) {
        ...
        if (userAgent.indexOf('iPad') > -1 || userAgent.indexOf('iPod') > -1 || userAgent.indexOf('iphone') > -1 || userAgent.indexOf('iPhone') > -1) {
            result.type = 'iOS';
            let version;
            try {
                version = userAgent.match(/(OS |os |OS_)(\d+((_|.)\d)?((_|.)\d)?)/)[0].replace('OS ', '').replace('_', '.');
                result.type += (` ${version}`);
            } catch (e) { }
            return result;
        }
    ...
    var createFile = function (content, name, type, obj) {
     ...
     * @param file: file
     */
    var checkFileExtension = function (extensions, file) {
    ...
    var isRtlLanguage = function () {
        return document.querySelector('html').getAttribute('lang') && document.querySelector('html').getAttribute('lang').toUpperCase() === 'FA';
    };

    var findByCustomID = function (parentContainer, value) {
    ...
    const scriptImporter = {
        url: (url) => {
            return new Promise((resolve, reject) => {
                ...
                script.addEventListener('error', () => {console.log("Error ==> " , url); reject(script)}, false);
