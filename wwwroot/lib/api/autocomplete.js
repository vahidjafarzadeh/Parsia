...
var Autocomplete = function () {
    ...
    var initStaticAutocomplete = function () {
        ...
        if (staticItem) { // agar baraye key morde-e nazar data vojood dashte bashad
            if (Util.differentTime(new Date().getTime(), staticItem.createdTime, config.autocompleteCacheTimeout !== null ? config.autocompleteCacheTimeout : Config.AUTOCOMPLETE_CACHE_TIMEOUT)) { // agar bazeye zamani b payan nareside bashad, data az storage khande shavad
    ...
    var fillStaticAutocomplete = function (data) {
        ...
        config.element.select2({
            ...
            .on('select2:select', function (e) {
                ...
                let itemData = $(e.params.data)[0];
                var object = optionElement.data('object');
                // multipleSelect
                if (config.multipleSelect) {
                    var wrapper = $(e.target).closest('.form-group').find('.multiple-select-wrapper');
            ...
            .on('change.select2', function (e) {
                ...
                if (config.multipleSelect && target.data() && target.data().multi) {
                    ...
                    var ctn = wrapper.find('.multiple-select-container');
                    let lstData = target.data().multi;
                    $.each(lstData, (i, itemData) => {
                        var isExist = false;
                        ctn.find('.badge').each(function () {
                            var ctx = $(this);
