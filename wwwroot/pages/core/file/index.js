...
const createFolder = (name, holder, data) => {
    ...
    container.dblclick(function() {
        ...
        where.add("parentId",
            `${data.entityId}`,
            ENVIRONMENT.Condition.EQUAL,
            ENVIRONMENT.Operator.AND,
            1,
            [],
            ENVIRONMENT.FieldType.LONG);
...
const createFile = (name, holder, data) => {
    let flg = false;
    $.each(localVariable.lstSelectFile,
        (i, v) => {
            ...
            if (container.hasClass("active")) {
                ...
                if (localVariable.lstSelectFile.length < localVariable.countSelectFile &&
                    ...
                    if (localVariable.allowExtensionSelect.length === 0) {
                        ...
                        $.each(localVariable.allowExtensionSelect,
                            (i, v) => {
                                if (v === data.extension) {
                                    flag = true;
                                }
    ...
    createNewFile.on("click",
        () => {
            ...
            if (fileName.val()) {
                ...
                Api.postForm({
                    url: localVariable.urls.createFile,
                    formData: formData,
                    isAsync: false,
                    handler: handler
                });
    ...
    filter.on("click",
        () => {
            ...
            where.add("displayInFileManager",
                ...
                [],
                ENVIRONMENT.FieldType.BOOLEAN);
            $.each($("#file-manager-filter-main-list input"),
                (index, item) => {
                    if ($(item).prop("checked")) {
                        flag = false;
                        const ext = $(item).attr("extension");
                        where.add("extension",
                            `${ext}`,
                            ENVIRONMENT.Condition.EQUAL,
                            ENVIRONMENT.Operator.OR,
                            1,
                            [],
