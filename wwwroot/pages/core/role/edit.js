...
var localVariables = {
    URLs: {
        services: {
            ...
            save: 'role/save',
            autocompleteView: "role/autocompleteView",
            AccessGroupList: "accessGroup/getAllData"
        }
    }
};
...
const getAllAccessGroup = () => {
    ...
    Api.post({
        url: localVariables.URLs.services.AccessGroupList,
        handler: handler,
        data: {}
    });
}
const createCheckBox = (data, selected) => {
    ...
    $.each(data, (index, value) => {
        let formGroup = $("<div/>", {
            class: "form-group floating-form-group col-12 col-lg-3"
        });
        let floating = $("<div/>", {
    ...
    if (selected && selected.length > 0) {
        $.each(selected, (i, v) => {
            $("#entity_" + v.entityId).prop("checked", true);
        });
    }
}
...
saveOrUpdate = function () {
    ...
    formData.append("accessGroup", child);
